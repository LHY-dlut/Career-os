...
export function Library() {
  ...
  const setFilter = (key: 'source' | 'category' | 'q' | 'page', value: string) => {
  ...
  if (!resourceId) return <LibraryOverview resources={libraryResources} sources={librarySources} sourceId={params.get('source') || ''} category={params.get('category') || ''} query={params.get('q') || ''} search={location.search} page={page} onFilter={setFilter} />;
    ...
    <div ref={readingRef} onScroll={updateActiveHeading} className="min-w-0 flex-1 overflow-y-auto overscroll-contain px-5 py-6 sm:px-8 sm:py-8">
      <div className="mx-auto max-w-3xl">
        ...
        <header className="mb-8 border-b border-slate-200 pb-7 dark:border-slate-800">
          ...
          <h1 className="break-words text-2xl font-bold leading-snug tracking-tight sm:text-3xl">{resource.title}</h1>
          {resource.summary && <p className="mt-4 text-sm leading-7 text-slate-500 dark:text-slate-400">{resource.summary}</p>}
