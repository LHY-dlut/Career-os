      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 rounded-2xl bg-gradient-to-br from-indigo-50 via-white to-blue-50/50 dark:from-[#131926] dark:via-[#10141e] dark:to-[#0f1422] border border-indigo-100/80 dark:border-indigo-950/60 shadow-xs">
        <div className="space-y-1">
          ...
          <h2 className="text-xl sm:text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-50">
            LLM & Algorithm Engineering Readiness
          </h2>
          <p className="text-xs sm:text-sm text-zinc-600 dark:text-zinc-400 max-w-2xl">
            Knowledge Learning → Question Bank → Coding Practice → Applications → Retrospective → Spaced Review
          </p>
        </div>
        ...
        <div className="flex items-center gap-2.5 shrink-0">
          ...
          >
            ...
            <span>Review {dueQuestions.length} Questions</span>
          </button>
          <button
            onClick={() => onNavigate('copilot')}
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 text-zinc-800 dark:text-zinc-200 hover:bg-zinc-50 dark:hover:bg-zinc-800 text-xs font-medium transition-colors"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          ...
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-xs text-zinc-500">
              ({masteredQuestions.length} Mastered)
            </span>
          </div>
        ...
        >
          ...
          <div className="mt-2 text-[11px] text-amber-600 dark:text-amber-400 flex items-center gap-1 font-medium">
            <span>Ready for spaced recall</span>
        ...
        >
          ...
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-xs text-zinc-500">
              ({applications.filter((a) => a.status === 'Interviewing').length} active)
            </span>
          </div>
          <div className="mt-2 text-[11px] text-zinc-500 flex items-center justify-between">
        ...
        >
          ...
          <div className="mt-2 text-[11px] text-zinc-500 truncate">
            {upcomingInterviews[0] ? (
              <span>Next: {upcomingInterviews[0].companyName}</span>
            ) : (
          ...
          <div className="p-5 rounded-xl bg-white dark:bg-[#12161f] border border-zinc-200 dark:border-zinc-800 shadow-xs">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">
                  Upcoming Interviews & Scheduled Rounds
                </h3>
              </div>
              <button
                onClick={() => onNavigate('interviews')}
            ...
            {upcomingInterviews.length === 0 ? (
              <div className="py-6 text-center text-xs text-zinc-500 bg-zinc-50 dark:bg-zinc-900/40 rounded-lg">
                No interviews scheduled for this week. Log a new round when recruiters reach out!
              </div>
            ) : (
                  ...
                  >
                    <div className="space-y-0.5">
                      ...
                      <p className="text-xs text-zinc-600 dark:text-zinc-400">
                        {inv.roundName} ({inv.durationMinutes} min)
                      </p>
                    </div>
          ...
          <div className="p-5 rounded-xl bg-white dark:bg-[#12161f] border border-zinc-200 dark:border-zinc-800 shadow-xs">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">
                  Questions to Review Today ({dueQuestions.length})
                </h3>
              </div>
              <button
                onClick={() => onNavigate('review')}
            ...
            {dueQuestions.length === 0 ? (
              <div className="py-6 text-center text-xs text-zinc-500 bg-zinc-50 dark:bg-zinc-900/40 rounded-lg">
                Great job! All spaced review questions are up to date.
              </div>
            ) : (
          ...
          <div className="p-5 rounded-xl bg-white dark:bg-[#12161f] border border-zinc-200 dark:border-zinc-800 shadow-xs">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
              {stages.map((st) => (
                ...
                >
                  <div className="text-[11px] text-zinc-500 mb-1">{st.label}</div>
        ...
        <div className="space-y-6">
          ...
          <div className="p-5 rounded-xl bg-white dark:bg-[#12161f] border border-zinc-200 dark:border-zinc-800 shadow-xs">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-[11px] text-zinc-500">
                {masteredQuestions.length}/{questions.length} Mastered
              </span>
            </div>
          ...
          <div className="p-5 rounded-xl bg-white dark:bg-[#12161f] border border-zinc-200 dark:border-zinc-800 shadow-xs">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-[11px] text-zinc-500">
                {codingAttempts.length} attempts
              </span>
            </div>
