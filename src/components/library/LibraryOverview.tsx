...
interface Props {
  ...
  search: string;
  page: number;
  onFilter: (key: 'source' | 'category' | 'q' | 'page', value: string) => void;
}

const PAGE_SIZE = 24;

export function LibraryOverview({ resources, sources, sourceId, category, query, search, page, onFilter }: Props) {
  const { t } = useI18n();
  ...
  const filtered = sourceResources.filter(resource => (!category || resource.category === category) && searchTerms.every(term => `${resource.title} ${resource.category} ${resource.tags.join(' ')} ${resource.summary} ${sources.find(source => source.id === resource.sourceId)?.name || ''}`.toLocaleLowerCase().includes(term)));
