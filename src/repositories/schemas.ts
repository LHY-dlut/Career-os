...
export const codingDraftSchema = z.object({ code: text, updatedAt: date, elapsedSeconds: nonnegative });
export const entitySchemas = {
  ...
  codingProblems: z.object({ ...base, title: text.min(1), category: text, difficulty, problemDescription: text, codeTemplate: text, referenceSolution: text, language: text, complexityAnalysis: text, keyPitfalls: strings, interviewExplanation: text, trainingTaskId: id.optional(), trainingTrack: z.enum(['hot100', 'pytorch']).optional(), drafts: z.record(z.string().min(1).max(32), codingDraftSchema).optional() }),
  codingAttempts: z.object({ ...base, problemId: id, problemTitle: text, attemptedAt: date, userCode: text, durationMinutes: nonnegative, status: z.enum(['Completed', 'Struggled', 'Partial', 'Abandoned']), selfRating: score, notes: text, language: text.optional(), errorReason: text.optional(), verification: z.enum(['not-run', 'local-samples-reported', 'leetcode-accepted-reported']).optional() }),
