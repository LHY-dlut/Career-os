# LoRA / PEFT Interview Cheat Sheet

> 来源：[ARIS-in-AI-Offer](https://github.com/wanshuiyin/ARIS-in-AI-Offer/blob/22e73822f8636ee3b52de1da6f6d2e03d1f51d4e/docs/tutorials/lora_peft_tutorial_en.md) · 作者：Ruofeng Yang（杨若峰）及 ARIS contributors · [MIT 许可](/library/licenses/aris-ai-offer-MIT.txt)
...
Below is the **real run** output of [`code/lora.py`](https://raw.githubusercontent.com/wanshuiyin/ARIS-in-AI-Offer/22e73822f8636ee3b52de1da6f6d2e03d1f51d4e/docs/tutorials/code/lora.py) ($\text{IN}=32, \text{OUT}=48, r=8, \alpha=16$) on **PyTorch 2.10 / CPU** (each line has an `assert`; the summary prints only if all pass):
...
The LoRA / DoRA core implementation of this tutorial has a minimal runnable version at [`docs/tutorials/code/lora.py`](https://raw.githubusercontent.com/wanshuiyin/ARIS-in-AI-Offer/22e73822f8636ee3b52de1da6f6d2e03d1f51d4e/docs/tutorials/code/lora.py):

- [`lora.py`](https://raw.githubusercontent.com/wanshuiyin/ARIS-in-AI-Offer/22e73822f8636ee3b52de1da6f6d2e03d1f51d4e/docs/tutorials/code/lora.py) — `LoRALinear` ($B=0$ identity start · $\alpha/r$ and rsLoRA $\alpha/\sqrt{r}$ scaling · merge/unmerge) + `DoRALinear` (magnitude–direction decomposition), with 6 `assert` sanity checks (start equivalence / $\Delta W \ne 0$ after one step / merge consistent & reversible / param count $= r(\text{in}+\text{out})$ / rsLoRA scaling / DoRA frozen base).
