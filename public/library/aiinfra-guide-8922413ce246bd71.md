# 3.4 Transformer前馈网络FFN深入理解

> 来源：[AIInfraGuide](https://github.com/caomaolufei/AIInfraGuide/blob/a3b63eeb81d6d36a3c42c8cfc5a1bdd96e36bab1/docs/guides/%E6%A8%A1%E5%9D%97%E4%B8%80-%E5%89%8D%E7%BD%AE%E7%9F%A5%E8%AF%86/transformer/3.4%20Transformer%E5%89%8D%E9%A6%88%E7%BD%91%E7%BB%9CFFN%E6%B7%B1%E5%85%A5%E7%90%86%E8%A7%A3.md) · 作者：草帽路飞（caomaolufei）及 AIInfraGuide contributors · [MIT 许可](/library/licenses/aiinfra-guide-MIT.txt)
...
- [1. FFN 在 Transformer 中的角色定位](#heading-1-ffn-在-transformer-中的角色定位)
- [2. FFN 的”展开-压缩”结构](#heading-2-ffn-的展开-压缩结构)
- [3. 激活函数详解](#heading-3-激活函数详解)
- [4. SwiGLU：当前大模型的主流选择](#heading-4-swiglu当前大模型的主流选择)
- [5. 参数量计算与对比](#heading-5-参数量计算与对比)
- [6. FFN 中间维度的选择：为什么是 4x 和 8/3x](#heading-6-ffn-中间维度的选择为什么是-4x-和-83x)
- [7. 张量并行切分 FFN](#heading-7-张量并行切分-ffn)
- [8. MoE：将 FFN 拆分为多个专家](#heading-8-moe将-ffn-拆分为多个专家)
- [9. CUDA Kernel 融合在 FFN 中的应用](#heading-9-cuda-kernel-融合在-ffn-中的应用)
...
将 `Swish(gate) * up` 融合为一个 kernel。两个 $(N, d_{ff})$ 的输入产生一个 $(N, d_{ff})$ 的输出，HBM 访问量从 5 次降为 3 次：未融合时读取 `gate` 和 `up`、写入并再次读取 Swish 的中间结果，最后写出 `hidden`；融合后只读取两个输入并写出一个输出，省去了中间结果的写入和读回。
