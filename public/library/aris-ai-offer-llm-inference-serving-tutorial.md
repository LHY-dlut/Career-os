...
> 来源：[ARIS-in-AI-Offer](https://github.com/wanshuiyin/ARIS-in-AI-Offer/blob/22e73822f8636ee3b52de1da6f6d2e03d1f51d4e/docs/tutorials/llm_inference_serving_tutorial.md) · 作者：Ruofeng Yang（杨若峰）及 ARIS contributors · [MIT 许可](/library/licenses/aris-ai-offer-MIT.txt)
...
stop string 跨 token 边界时，服务器必须在推给客户端前**缓冲一段"可能仍是 stop string 前缀"的未提交文本**，确认不会再匹配到 stop string 才能安全吐出（§8 [D03] 给出最小实现）。这与 tokenization 教程的 **prefix instability** 同属"边界不稳定"问题但发生在不同层面——一个在 tokenizer 的 encode 边界，一个在流式输出的字符串匹配边界；前者与 token healing 的完整机制见 [`tokenization_tutorial.md`](/library/aris-ai-offer-tokenization-tutorial) §5.4。

## §4 KV 生命周期：分配、分页、共享与路由

### 4.1　容量核算：KV 字节数进入 serving 之后

每 token 的 KV 字节数直接复用 [`kv_cache_speculative_decoding_tutorial.md`](/library/aris-ai-offer-kv-cache-speculative-decoding-tutorial) §2.1 的结论：
...
核心机制（完整设计见 [`kv_cache_speculative_decoding_tutorial.md`](/library/aris-ai-offer-kv-cache-speculative-decoding-tutorial) §5）：每个序列的逻辑 KV block 通过一张 **block table** 映射到显存里不连续的物理 block，attention kernel 按这张表间接寻址读取 KV。固定大小 block 消除了"每个请求必须预留整段连续最大长度 KV 空间"的要求，降低外部碎片与过度预留；但**最后一个未填满的 block 仍有内部碎片**（§8 [D07] 的分配器演示）。
...
推理阶段没有 backward、没有 optimizer state，但这不代表并行通信消失了——通信量分析、backward/ZeRO/FSDP/训练 pipeline schedule 的完整推导见 [`distributed_training_tutorial.md`](/library/aris-ai-offer-distributed-training-tutorial)，这里只讲推理特有的三条：
...
- **量化**：权重量化主要降低权重的带宽与容量压力；**KV-cache 量化是独立的设计点**，不能因为模型权重用了低比特就假定 KV 也理所当然用相同比特数（数值敏感度、per-channel/per-token 粒度选择完全不同）。数值细节（scale/zero-point、GPTQ/AWQ/FP8 具体误差分析）见 [`quantization_tutorial.md`](/library/aris-ai-offer-quantization-tutorial)，本节只给决策维度：
...
- **Speculative decoding**：接受率、$E[\tau]$ 期望加速比的完整推导见 [`kv_cache_speculative_decoding_tutorial.md`](/library/aris-ai-offer-kv-cache-speculative-decoding-tutorial) §7；serving 侧要记的是：每次 target verification 会处理多个候选 token，产生**可变数量**的已接受 token，调度器必须按"验证 token 数"“输出 token 数”“KV 增量”分别记账，不能当成固定步长的 decode。Target 或 draft 的量化可能通过数值误差改变 draft/target 的分布差异与接受率——**speculative decoding 与量化不严格正交**。
...
完整可跑脚本见 [`code/inference_serving.py`](https://raw.githubusercontent.com/wanshuiyin/ARIS-in-AI-Offer/22e73822f8636ee3b52de1da6f6d2e03d1f51d4e/docs/tutorials/code/inference_serving.py)（纯标准库、无第三方依赖、CPU 秒级跑完 [D01]–[D09]），每个 demo 验证一个孤立机制：
