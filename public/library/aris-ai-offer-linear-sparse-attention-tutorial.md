# 线性 / 稀疏注意力 面试 Cheat Sheet

> 来源：[ARIS-in-AI-Offer](https://github.com/wanshuiyin/ARIS-in-AI-Offer/blob/22e73822f8636ee3b52de1da6f6d2e03d1f51d4e/docs/tutorials/linear_sparse_attention_tutorial.md) · 作者：Ruofeng Yang（杨若峰）及 ARIS contributors · [MIT 许可](/library/licenses/aris-ai-offer-MIT.txt)
...
本 tutorial 的代码（§6 的 `linear_attn_recurrent` / `linear_attn_chunkwise`，以及下方 delta-rule 与 block-sparse 片段）应满足以下关键不变量。完整可跑脚本见 [`code/linear_sparse_attention.py`](https://raw.githubusercontent.com/wanshuiyin/ARIS-in-AI-Offer/22e73822f8636ee3b52de1da6f6d2e03d1f51d4e/docs/tutorials/code/linear_sparse_attention.py)（纯 PyTorch、CPU 几秒、6 个 assert）——**真实运行输出附在本节末**。
