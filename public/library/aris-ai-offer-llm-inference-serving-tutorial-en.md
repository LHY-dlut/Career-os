# LLM Inference & Serving Stack Interview Cheat Sheet

> 来源：[ARIS-in-AI-Offer](https://github.com/wanshuiyin/ARIS-in-AI-Offer/blob/22e73822f8636ee3b52de1da6f6d2e03d1f51d4e/docs/tutorials/llm_inference_serving_tutorial_en.md) · 作者：Ruofeng Yang（杨若峰）及 ARIS contributors · [MIT 许可](/library/licenses/aris-ai-offer-MIT.txt)
...
Inference has no backward pass and no optimizer state, but that does not make parallel communication disappear — communication-volume analysis and the full derivations of backward/ZeRO/FSDP/training pipeline schedules are in [`distributed_training_tutorial.md`](/library/aris-ai-offer-distributed-training-tutorial); here are only the three inference-specific points:
...
- **Quantization**: weight quantization mainly relieves weight bandwidth and capacity pressure; **KV-cache quantization is an independent design point** — the model using low-bit weights does not entitle the KV to the same bit width as a matter of course (numeric sensitivity and per-channel/per-token granularity choices differ completely). Numeric details (scale/zero-point, concrete GPTQ/AWQ/FP8 error analysis) are in [`quantization_tutorial.md`](/library/aris-ai-offer-quantization-tutorial); this section gives only the decision dimensions:
...
The full runnable script is [`code/inference_serving.py`](https://raw.githubusercontent.com/wanshuiyin/ARIS-in-AI-Offer/22e73822f8636ee3b52de1da6f6d2e03d1f51d4e/docs/tutorials/code/inference_serving.py) (pure standard library, no third-party dependencies, [D01]–[D09] finish in seconds on CPU); each demo verifies one isolated mechanism:
