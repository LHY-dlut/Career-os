...
> 来源：[ARIS-in-AI-Offer](https://github.com/wanshuiyin/ARIS-in-AI-Offer/blob/22e73822f8636ee3b52de1da6f6d2e03d1f51d4e/docs/tutorials/rag_embedding_retrieval_tutorial.md) · 作者：Ruofeng Yang（杨若峰）及 ARIS contributors · [MIT 许可](/library/licenses/aris-ai-offer-MIT.txt)
...
下面是 [`code/rag_embedding.py`](https://raw.githubusercontent.com/wanshuiyin/ARIS-in-AI-Offer/22e73822f8636ee3b52de1da6f6d2e03d1f51d4e/docs/tutorials/code/rag_embedding.py) 在 **PyTorch 2.10 / CPU** 上的**真实运行**输出（每行带 `assert`，全过才打印汇总）：
...
本 tutorial 的嵌入 / 检索核心在 [`docs/tutorials/code/rag_embedding.py`](https://raw.githubusercontent.com/wanshuiyin/ARIS-in-AI-Offer/22e73822f8636ee3b52de1da6f6d2e03d1f51d4e/docs/tutorials/code/rag_embedding.py) 有最小可跑版本：

- [`rag_embedding.py`](https://raw.githubusercontent.com/wanshuiyin/ARIS-in-AI-Offer/22e73822f8636ee3b52de1da6f6d2e03d1f51d4e/docs/tutorials/code/rag_embedding.py) — `DualEncoder`（L2 归一化双塔）+ `info_nce_in_batch` / `info_nce_with_hard`（in-batch + 难负例对比损失）+ `bm25_scores` / `rrf` / 混合召回，含 6 个 `assert` sanity check（对角线即正例 / 归一化 / 温度锐度 / with-hard 形状 / RRF 单调 / 混合保留实体块）。
