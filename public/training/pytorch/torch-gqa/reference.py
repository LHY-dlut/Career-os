...
def grouped_query_attention(q, k, v, allowed=None):
    """Q[B,Hq,Q,D], K[B,Hkv,K,D], V[B,Hkv,K,Dv], consecutive Q-head groups share KV."""
    if q.ndim != 4 or k.ndim != 4 or v.ndim != 4:
        raise ValueError("Q/K/V must have rank four")
    hq, hkv = q.shape[1], k.shape[1]
    if hkv <= 0 or hq <= 0 or hq % hkv or v.shape[1] != hkv:
        raise ValueError("Hq must be a positive multiple of matching K/V head counts")
    repeats = hq // hkv
    # Materialization makes the grouping explicit for teaching, not an optimized kernel.
    expanded_k = k.repeat_interleave(repeats, dim=1)
    expanded_v = v.repeat_interleave(repeats, dim=1)
    return scaled_attention(q, expanded_k, expanded_v, allowed)
